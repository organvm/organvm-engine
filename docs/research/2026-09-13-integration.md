...
The original September 13 isolated run remains historical: 170 tests passed on
Python 3.13.5, without the full Engine checkout or repository-native static checks.
It is not the current acceptance result.

The final exact-head run was executed in a complete Engine checkout on September 18
with Python 3.12.14: **326 passed, zero failures/errors/skips**. Ruff passed and
Pyright returned zero errors (with environment-only missing-source/import warnings).
The committed JUnit artifact was generated by the exact command recorded in
`evidence/integration-2026-09-13.json`; the receipt binds its digest together with
the source/test files and captured text log.

Hosted required checks, independent current-head review, trusted scheduled ingestion,
deployment, and the supported Python-version matrix remain outstanding. The local
result does not substitute for those release gates or authenticate its own producer.
...
This command was executed in the complete checkout described above. Follow CONTRIBUTING
and the actual CI contract for the complete supported matrix. Do not treat `done.sh` as a
